...
class Encoder(nn.Module):
    def __init__(self, hidden_size, height, width, bgru, step, flip):
        ...
        self.fc_batch1 = nn.Sequential(
                nn.Linear(512*4*13, 8192), 
                #nn.BatchNorm1d(8192),                               
                nn.Linear(8192, 4096),
                #nn.BatchNorm1d(4096),                               
    ...
    # (32, 1, 80, 1400)
    def forward(self, in_data, img_width, mode, hidden=None):
        in_data_len = img_width
        batch_size = in_data.shape[0]
        if mode == 'con_enc':
            in_data = torch.cat([in_data,in_data,in_data], dim=1)
            xx = self.layer(in_data) # torch.Size([32, 512, 4, 63])
            xxx = xx.reshape(batch_size, -1)
            if batch_size == 1:
                xxxx = self.fc_batch1(xxx)
            else:
                xxxx = self.fc(xxx)
            return xx, xxxx
        elif mode == 'image':
            ...
            print('illegal mode input for encoder')
            exit()
